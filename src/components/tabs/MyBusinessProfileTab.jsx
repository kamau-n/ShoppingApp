...
const MyBusinessProfileTab = ({ myProfile, handleEdit }) => {
  ...
  const formatHours = (hours) => hours || "Closed";
  ...
  const enhancedProfile = {
    ...myProfile,
    description:
      "Book Shelve is a cozy independent bookstore offering a carefully curated selection of books across all genres. We pride ourselves on our knowledgeable staff, community events, and comfortable reading spaces.",
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 w-full max-w-4xl mx-auto">
      ...
      <div className="flex justify-end p-4 border-b">
        <button
          onClick={() => handleEdit(enhancedProfile)}
        ...
        <div className="overflow-x-auto p-6 space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            {enhancedProfile.logo_url ? (
              <img
                src={enhancedProfile.logo_url}
                alt={`${enhancedProfile.business_name} logo`}
                className="w-32 h-32 rounded-lg object-cover border"
              />
            ) : (
              <div className="w-32 h-32 rounded-lg bg-blue-100 flex items-center justify-center border">
                <ShoppingBag className="w-16 h-16 text-blue-600" />
              </div>
            )}
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-gray-900">
                {enhancedProfile.business_name}
              </h2>
              <div className="mt-2 flex flex-wrap gap-2">
                {enhancedProfile.categories?.map((c, i) => (
                  <span
                    key={i}
                ...
                <span
                  ...
                  {enhancedProfile.subscription_status}
                </span>
              </div>
            ...
            {[
              {
                ...
                label: "Founded",
                value: enhancedProfile.founded_year,
              },
              {
                icon: <Users className="w-5 h-5" />,
                label: "Team Size",
                value: enhancedProfile.team_size,
              },
              {
                icon: <Calendar className="w-5 h-5" />,
                label: "Member Since",
                value: formatDate(enhancedProfile.created_at),
              },
          ...
          {/* Locations */}
          {enhancedProfile.locations?.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">
                Locations
              </h3>
              <div className="grid grid-cols-1 gap-4">
                {enhancedProfile.locations.map((loc) => (
                  <div key={loc.id} className="p-4 border rounded-lg">
                    ...
                    {loc.phone && (
                      <div className="mt-2 flex items-start gap-2">
                        <Phone className="w-5 h-5 text-gray-500 mt-0.5" />
                        <p className="text-gray-700">{loc.phone}</p>
                      </div>
                    )}
                  </div>
                ))}
          ...
          {enhancedProfile.working_hours && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(enhancedProfile.working_hours).map(
                  ([day, hours]) => (
                    <div
                      key={day}
